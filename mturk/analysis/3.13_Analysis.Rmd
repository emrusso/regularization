...
aggregate_data <- comparison_data %>%
  filter(abs(centered_pos) <= 3) %>%
  mutate(word_length = 1,#nchar(gsub("[^A-Za-z]+","",stimulus)),
         log_rt_per_char = exp(log_rt) / word_length) %>%
  ...
  group_by(version, pos, trial, subj, utterance_in_exchange, 
            true_display_order) %>%
   summarise(log_rt_per_char = mean(log_rt_per_char), na.rm = T)

char_model <- lmer(log_rt_per_char ~ version * pos + 
...
plot_data <- aggregate_data %>%
  ...
  summarise(log_rt_per_char = mean(log_rt_per_char, na.rm = T)) %>%
  summarise(log_rt_per_char = mean(log_rt_per_char, na.rm = T)) %>%
  summarise(sem = sqrt(var(log_rt_per_char, na.rm = T) / 
                         sum(!is.na(log_rt_per_char))),
            empirical_stat = mean(log_rt_per_char, na.rm = T)) %>%
  mutate(ci_upper = empirical_stat + sem, ci_lower = empirical_stat - sem)
  #tidyboot_mean(log_rt_per_char)
