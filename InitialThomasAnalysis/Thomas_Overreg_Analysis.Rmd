...
ggplot(thomas_or_rates, aes(x=age, y=or_rate)) + geom_line() + geom_smooth()

ggplot(thomas_or_rates, aes(x=age, y=error_rate)) + geom_line() + geom_smooth()
...
ac_utts <- thomas_data %>%
  ...
  mutate(split=strsplit(utterance, " ")) %>%
  group_by(utterance) %>%
  mutate(ul=as.numeric(length(split)))
