...
thomas_data <- read.csv("thomas_utterance_data.csv")
...
thomas_kid_with_overregs <- read.csv("all_thomas_kid_utterances_with_overregs.csv")
...
utt_to_sf <- utt_to_sf[,c("rowname", "source_file", "utterance_start", "utterance_end", "response_start", "response_end")]
...
fixed_overregs <- read.csv("final_audio_coding.csv")
...
thomas_kid_overreg_free <- rownames_to_column(thomas_kid_with_overregs) %>%
  filter(!overreg)

together <- full_join(fixed_overregs_cleaner, thomas_kid_overreg_free, by = "rowname")
...
together <- together %>%
  ...
  filter(is.na(response_time) | response_time != ugh) %>%
  filter(is.na(utterance_time) | utterance_time != ugh) %>%
  mutate(response_time = ifelse(is.na(response_time), as.numeric(as.character(response_start)) - as.numeric(as.character(utterance_end)), as.numeric(as.character(response_time)))) %>%
  ...
  mutate(utterance_time = ifelse(is.na(utterance_time), as.numeric(as.character(utterance_end)) - as.numeric(as.character(utterance_start)), as.numeric(as.character(utterance_time))))

together <- together[,c("utterance", "response", "speaker", "responder", "error", "age", "overreg", "response_time", "utterance_time")]
together <- together %>%
  mutate_at(vars(error, overreg), as.logical)
